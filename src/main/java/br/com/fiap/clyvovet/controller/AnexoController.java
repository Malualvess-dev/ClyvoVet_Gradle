...
public class AnexoController {
    ...
    @GetMapping("/{id}")
    public ResponseEntity<AnexoResponse> readAnexo(
            @PathVariable Integer id
    ) {

        return new ResponseEntity<>(
                anexoService.readAnexo(id),
                HttpStatus.OK
        );
    }
    ...
    @GetMapping
    public ResponseEntity<Page<AnexoResponse>> readAnexos(
            @ParameterObject Pageable pageable
    ) {

        return new ResponseEntity<>(
                anexoService.read(pageable),
                HttpStatus.OK
        );
    }
    ...
    @GetMapping("/tipo")
    public ResponseEntity<Page<AnexoResponse>> readByTipoArquivo(
            @RequestParam TipoArquivo tipoArquivo,
            @ParameterObject Pageable pageable
    ) {

        return new ResponseEntity<>(
                anexoService.readByTipoArquivo(
                        tipoArquivo,
                        pageable
                ),
                HttpStatus.OK
    ...
    @PutMapping("/{id}")
    public ResponseEntity<AnexoResponse> updateAnexo(
            @PathVariable Integer id,
            @Valid @RequestBody AnexoRequest anexoRequest
    ) {

        return new ResponseEntity<>(
                anexoService.update(id, anexoRequest),
                HttpStatus.OK
        );
    }
